...
function login(loginData) {
	...
	return fetch(apiBaseURL + "/auth/login", options).then((response) => {
		if (!response.ok) {
		...
		document.getElementById("displayNotif").textContent = "Incorrect username/password..";
	}).finally(() => {
...
function logout() {
	...
	fetch(apiBaseURL + "/auth/logout", options).then(response => response.json()).then(data => console.log(data)).finally(() => {
